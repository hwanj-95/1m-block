...
int Url_filter(unsigned char* data){
    ...
    char data_buf[1000];
    char url_data[1000]; //text file url
        ...
        for(int i = 0; i<tcp_payload; i++ ){
            if(data[i] == 0x0d && data[i+1] == 0x0a && data[i+2] == 0x48 && data[i+3] == 0x6f &&
               data[i+4] == 0x73 && data[i+5] == 0x74 && data[i+6] == 0x3a && data[i+7] == 0x20){
                ...
                //return check = strncmp(HostAddr, data_buf, strlen(data_buf)); // packet test.gilgil.net.naver.com // test.gilgil.net

                FILE* file = fopen(site_list,"r");
                if(file == NULL){
                ...
                while(!feof(file)){
                    fgets(url_data, sizeof(data), file);
                    check = strncmp(data_buf, url_data, strlen(url_data));
                    if(check==0){
                        fclose(file);
                        break;
                    }
